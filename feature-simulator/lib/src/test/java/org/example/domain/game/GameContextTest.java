...
import org.example.domain.model.state.SingleBasesState;
...
public class GameContextTest {
    ...
    public void updateBaseStateTest(String description, org.example.domain.model.state.BasesState newState) {
        // given
        GameContext gameContext = new GameContext(Mock_Batters.mock());
    ...
    public void setRunnerToTest(String description, int baseNumber, boolean hasBatter, int expectedBaseCount) {
        // given
        GameContext gameContext = new GameContext(Mock_Batters.mock());
        Optional<org.example.domain.model.player.Batter> batter = hasBatter ? Optional.of(Mock_Batters.mock().get(0)) : Optional.empty();

        // when
        gameContext.setRunnerTo(baseNumber, batter);

        // then
        Optional<org.example.domain.model.player.Batter> runner = switch (baseNumber) {
    ...
    static Stream<Arguments> updateBaseStateOfTestCases() {
        return Stream.of(
                arguments(
                        ...
                        true, true, true,
                        org.example.domain.model.state.FullBasesState.class
                ),
                arguments(
                        "[分岐2] 1,2塁 → FirstDoubleBaseState",
                        true, true, false,
                        org.example.domain.model.state.FirstDoubleBaseState.class
                ),
                arguments(
                        "[分岐3] 1,3塁 → FirstThirdBaseState",
                        true, false, true,
                        org.example.domain.model.state.FirstThirdBaseState.class
                ),
                arguments(
                ...
                arguments(
                        ...
                        false, true, true,
                        org.example.domain.model.state.DoubleThirdBaseState.class
                ),
                arguments(
                        "[分岐6] 2塁のみ → DoubleBaseState",
                        false, true, false,
                        org.example.domain.model.state.DoubleBaseState.class
                ),
                arguments(
                        "[分岐7] 3塁のみ → ThirdBaseState",
                        false, false, true,
                        org.example.domain.model.state.ThirdBaseState.class
                ),
                arguments(
